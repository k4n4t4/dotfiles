...
function M.lsp_picker()
    ...
    snacks.picker.select(items, {
        ...
        if not client then
            vim.notify("No LSP client found with ID: " .. item.id, vim.log.levels.WARN, { title = "LSP List" })
            return
        end
        ...
        snacks.picker.select(sub_items, {
            ...
            if sub_item.text == "Stop LSP" then
                vim.notify("Stopping LSP: " .. client.name, vim.log.levels.INFO, { title = "LSP List" })
                client:stop()
            elseif sub_item.text == "Restart LSP" then
                vim.notify("Restarting LSP: " .. client.name, vim.log.levels.INFO, { title = "LSP List" })
                client:_restart()
            end
        end)
    end)
...
function M.lsp_picker_all()
    ...
    snacks.picker.select(items, {
        format_item = function(item)
            ...
            local file_types = item.config and item.config.filetypes and table.concat(item.config.filetypes, ", ")
                or "N/A"
